//from https://gist.github.com/shussekaido/4216123
//remember to change imac-tophe.local by yours
...
function ourAddShard(setname,port) {
	...
	res = rs.initiate(
		{
			"_id" : setname,
			"members" : [
				{ _id: 0, host: "imac-tophe.local:" + port },
				{ _id: 1, host: "imac-tophe.local:" + (port+1) },
				{ _id: 2, host: "imac-tophe.local:" + (port+2) }
			]
		}
	);
	check();
	...
	print("adding shard...");
	db = mongosConn;
	res = sh.addShard(setname+"/imac-tophe.local:"+port);
	check();
